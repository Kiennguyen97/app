...
namespace VBA\CategorySeparated\Block\Product;
...
class ListProduct  extends ListProductParent
{
   /**
    ...
    public function getFulltextCollection(){
        if(!$this->getData('fulltextproducts')){
            $category = $this->getLayer()->getCurrentCategory();
            $all_children = $category->getChildren();
            $collection = $this->_fulltext->addCategoriesFilter(['in'=> $all_children]);
            $this->setData('fulltextproducts',$collection);
        }
        return $this->getData('fulltextproducts');
    }
    public function getAtrCustom(){
        $attributeCode='type';
        if ($attributeCode != null) { 
            $options = $this->getFulltextCollection()->getFacetedData($attributeCode);
            $ids = [];
            ...
            $options = $attribute->getSource()->getSpecificOptions($ids);
            $result = [];
            foreach ($options as $option) {
    ...
    /*funtion get productby type*/
    public function getProductCollectionByType()
    {
       
        $product =  $this->getLayer()->getProductCollection();
        
        $atribute_code ='type';
        $type_atribute = $this->getAtrCustom();
        $product_collection_filters_type = [];
        
        foreach ($type_atribute as $value) {
            print_r($value['value']);
            if(!array_key_exists($value['label'], $product_collection_filters_type)){
                $product_collection_filters_type[$value['label']] = [];
            }
            $product_collection_filters_type[$value['label']][] =  $product->addAttributeToFilter($atribute_code, $value['value']);
            // $this->_getProductCollection()->addAttributeToFilter($atribute_code, $value['value'])->getSelect();
            // print_r('<br>');
        }
        return  $product_collection_filters_type;
    }
    ...
    private function initializeProductCollection()
    {
        ...
        if ($this->getLayer()->getCurrentCategory()->getData('cat_separated')==1) {
            //  $joinConditions = 'e.type = eav_attribute_option.option_id';
            // $collection->getSelect()->join(
            //     ['eav_attribute_option'],
            //     $joinConditions,
            //     []
            // )->columns("eav_attribute_option.sort_order")->order('sort_order ASC');
            $joinConditions1='e.entity_id = catalog_product_flat_1.entity_id';
             $collection->getSelect()->join(
                ['catalog_product_flat_1'],
                $joinConditions1,
                []
            )->columns("catalog_product_flat_1.type");
            //echo  $collection->getSelect();die; 
            $joinConditions = 'catalog_product_flat_1.type = eav_attribute_option.option_id';
            $collection->getSelect()->join(
                ['eav_attribute_option'],
                $joinConditions,
                []
